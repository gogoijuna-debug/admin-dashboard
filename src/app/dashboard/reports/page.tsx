...
export default function ReportsPage() {
  ...
  const totalRevenue =
    currentSales.reduce((sum, s) => sum + Number(s.totalAmount || 0), 0) +
  ...
  const chartData = Array.from({ length: 7 }, (_, i) => {
    ...
    return {
      ...
      revenue: daySales.reduce((sum, s) => sum + Number(s.totalAmount || 0), 0),
    };
  });
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          ...
          <div key={i} className="bg-white dark:bg-slate-900 p-8 rounded-[35px] border border-slate-100 dark:border-slate-800 shadow-sm">
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-6 ${statStyles[s.color] || statStyles.emerald}`}>
              <s.icon size={24} />
            </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[40px] border border-slate-100 dark:border-slate-800 shadow-sm">
          ...
          <div className="space-y-6">
            ...
            <div className="p-6 bg-slate-50 dark:bg-slate-800/50 rounded-3xl border border-slate-100 dark:border-slate-800/30">
              ...
              <p className="text-2xl font-black text-slate-900 dark:text-white">₹{currentSales.reduce((sum, s) => sum + Number(s.totalAmount || 0), 0)}</p>
            </div>

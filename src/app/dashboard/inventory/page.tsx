    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="bg-white dark:bg-slate-900 p-8 rounded-[40px] border border-slate-100 dark:border-slate-800 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
          ...
          <div className="flex items-center gap-2 p-1 bg-slate-50 dark:bg-slate-950 rounded-2xl overflow-x-auto">
            {["All", "Medicine", "Feed"].map(c => (
              <button 
                key={c} onClick={() => setCategoryFilter(c as any)}
                className={`px-4 md:px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap shrink-0 ${categoryFilter === c ? 'bg-white dark:bg-slate-900 text-slate-900 dark:text-white shadow-md' : 'text-slate-400'}`}
              >
                {c}
              </button>
            ))}
        ...
        <div className="lg:hidden space-y-3">
          {filteredItems.map(item => (
            <div key={item.id} className="rounded-3xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 p-4">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 bg-slate-100 dark:bg-slate-800 rounded-xl flex items-center justify-center text-slate-400 overflow-hidden border border-slate-100 dark:border-slate-800">
                  {item.imageUrl ? (
              ...
              <div className="grid grid-cols-2 gap-3 mt-4 text-[11px]">
                <div>
                  <p className="text-slate-400 font-black uppercase tracking-widest text-[9px]">Stock</p>
                  <p className="font-black text-slate-700 dark:text-slate-300">{item.stock} {item.unit}</p>
                </div>
                <div>
                  <p className="text-slate-400 font-black uppercase tracking-widest text-[9px]">Unit Price</p>
                  <p className="font-black text-slate-900 dark:text-white">₹{item.price}</p>
                </div>
                <div>
                  <p className="text-slate-400 font-black uppercase tracking-widest text-[9px]">MRP</p>
                  <p className="font-bold text-slate-500">₹{item.mrp || item.price}</p>
                </div>
                <div>
                  <p className="text-slate-400 font-black uppercase tracking-widest text-[9px]">Total Value</p>
                  <p className="font-black text-emerald-600">₹{(item.price * item.stock).toLocaleString()}</p>
                </div>
              </div>

              <div className="flex items-center justify-end gap-2 mt-4">
                <button onClick={() => handleEdit(item)} className="px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 text-[10px] font-black uppercase tracking-widest text-slate-600 dark:text-slate-200 inline-flex items-center gap-2"><Edit2 size={14} />Edit</button>
                <button onClick={() => handleDelete(item.id)} className="px-3 py-2 rounded-xl border border-red-200 dark:border-red-900/40 text-[10px] font-black uppercase tracking-widest text-red-600 inline-flex items-center gap-2"><Trash2 size={14} />Delete</button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="hidden lg:block overflow-x-auto">
          <table className="w-full min-w-[980px] text-left border-separate border-spacing-y-3">
            <thead>
